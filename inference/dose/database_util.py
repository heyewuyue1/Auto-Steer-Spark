    ...
    with open(workload_file_name + ".bitmaps", 'rb') as f:
        for i in range(len(tables)):
            ...
            if not four_bytes:
                ...
                exit(1)
            num_bitmaps_curr_query = int.from_bytes(four_bytes,
                                                    byteorder='little')
            bitmaps = np.empty(
                (num_bitmaps_curr_query, num_bytes_per_bitmap * 8),
                dtype=np.uint8)
...
def formatJoin(json_node):
    ...
    ## TODO: index cond
    elif 'Index Cond' in json_node and not json_node['Index Cond'][
            -2].isnumeric():
    ...
    if join is not None:
        ...
        twoCol = [
            json_node['Alias'] + '.' + col if len(col.split('.')) == 1 else col
            for col in twoCol
        ]
...
def formatFilter(plan):
    ...
    filters = []
    if 'Filter' in plan:
        for filt in plan['Filter'].split('AND'):
            filters.append(filt.strip())
...
class Encoding:
    ...
    def __init__(self,
        ...
        self.idx2op = {0: '>', 1: '=', 2: '<',  3: '>=', 4: '<=', 5: 'isnotnull', 6: 'isnull', 7: 'NA'}
    ...
    def encode_filters(self, filters=[], alias=None):
        ...
        for f in filters:
            ...
            # print(res)
            # assert len(res['colId']) == len(res['opId']) and len(res['colId']) == len(res['val'])
        return res
...
class TreeNode:
    ...
    def __init__(self, nodeType, typeId, filt, card, join, join_str,
                 filterDict, pos):
