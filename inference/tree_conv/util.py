...
def _flatten(root, transformer, left_child, right_child):
    ...
    # return np.concatenate((np.array(root[0], ndmin=2), _flatten(root[1], transformer, left_child, right_child), _flatten(root[2], transformer, left_child, right_child))) if len(root) == 3 else np.array(root, ndmin=2)
