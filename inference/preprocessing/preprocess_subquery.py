...
def PlanToTree(plan):
    ...
    Htree = Str_Sub_Tree(max_number,node_num,lines[:parent_index])
    ...
    for id_value,idx in id_indices:
        ...
        if id_value in result:
            if not isinstance(result[id_value], list):
                result[id_value] = [result[id_value]]
    ...
    for P_idx in flag.keys():
        if  isinstance(flag[P_idx], list):
            i = 0
            for q in flag[P_idx]:
                Tree = Str_Sub_Tree(max_number,int(q),
                                    result[P_idx][i])
                if Tree[0].lc is None:
                    continue
                if Htree[int(P_idx)].rc is None:
                    Htree[int(P_idx)].rc = Tree[0].lc
                else:
                    Htree[Tree[0].lc+1]=Node(Tree[0].lc+1,'root')
                    Temp =Htree[int(P_idx)].rc
                    Htree[int(P_idx)].rc = Tree[0].lc+1
                    Htree[Tree[0].lc+1].lc = Temp
                    Htree[Tree[0].lc+1].rc = Tree[0].lc
                preorder_traversal(Htree,Tree,Tree[0].lc)
                i +=1
        else:
            Tree = Str_Sub_Tree(max_number,int(flag[P_idx]),result[P_idx])
            if Htree[int(P_idx)].rc is None:
                Htree[int(P_idx)].rc = Tree[0].lc
                preorder_traversal(Htree,Tree,Tree[0].lc)
            else:
...
def Str_Sub_Tree(max_number,node_num,lines):
